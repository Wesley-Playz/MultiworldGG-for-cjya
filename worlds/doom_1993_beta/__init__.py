...
from .options import DOOM1993OptionGroups, DOOM1993Options

game_name = "DOOM 1993 (Beta)"


class DOOM1993Location(AP.Location):
    game: str = game_name


class DOOM1993Item(AP.Item):
    game: str = game_name


class DOOM1993Web(WebWorld):
    option_groups = DOOM1993OptionGroups
    rich_text_options_doc = True
    bug_report_page = "https://github.com/ArchipelagoDoom/APDoom/issues"
    theme = "dirt"
    tutorials = [AP.Tutorial(
        "Multiworld Setup Guide",
        f"A guide to setting up the {game_name} randomizer connected to MultiworldGG.",
        "English",
        "setup_en.md",
        "setup/en",
...
class DOOM1993World(id1CommonWorld, import_data=(__name__, "doom.data.json")):
    """
    ...
    options: DOOM1993Options  # type: ignore
    web = DOOM1993Web()
    ...
    def create_item(self, name: str) -> DOOM1993Item:
        ...
        return DOOM1993Item(name, classification, item_id, self.player)

    def generate_early(self) -> None:
        self.init_episodes()

        ######## Custom code for this world begins here ########
        # Give the generator a clue on how to proceed out of solo Episode 1
        if self.solo_episode(1):
            early_level = self.random.choice(["Nuclear Plant (E1M2)", "Toxin Refinery (E1M3)", "Command Control (E1M4)",
                                              "Phobos Lab (E1M5)", "Military Base (E1M9)"])
            self.multiworld.early_items[self.player][early_level] = 1

        # Solo Episode 3 presents a much bigger problem, because Hell Keep has only two locations.
    ...
    def create_regions(self) -> None:
        ...
        self.make_regions(location_type=DOOM1993Location)

    def set_rules(self) -> None:
        self.make_rules()

    def create_items(self) -> None:
        self.place_level_complete_items(item_type=DOOM1993Item)
